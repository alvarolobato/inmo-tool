...
| `cli/commands/` | Command implementations. `sql.sh`/`wren.sh` (4D/WrenAI-specific) were removed in task 1.1; `prod.sh` came back in Aug 2026 for this project's own deployment, sharing nothing with the source project's version. `connector.sh`/`db.sh`/`dedup.sh` (task 1.5, #13) replaced the placeholder `etl.sh` — see `docs/skills/cli.md`. |
...
| `deploy/` | Production deployment scripts (host bootstrap, data copy, psql/pg_dump against the shared cluster). Run over SSH by `ps prod`. |
| `config/production.md` | How production is deployed and where its settings live. Deliberately names no host, address or port — see the note there. |
| `docker-compose.prod.yml` | Production stack: `db-init` + `otel-collector` + `etl` + `dashboard`, and **no PostgreSQL** — it connects to a cluster already running on the host. |
...
### Production settings — not in this repository

There is a production deployment, driven by `ps prod ...` over SSH. **Nothing
about the host is committed**: no address, no hostname, no port, no mention of
what else runs on that machine. This repo is public, so treat any of that as
credential-grade and keep it out of committed files, commit messages and
issues.

Where to find it instead, when a task actually needs it:

| What | Where |
|------|-------|
| ssh target and deployment path | `PROD_HOST` / `PROD_PATH` in `~/.config/inmo-tool/.env` |
| shared-PostgreSQL network, host, dashboard port | `SHARED_NETWORK` / `POSTGRES_HOST` / `DASHBOARD_PORT`, same file |
| the deployment's own secrets | `<PROD_PATH>/.env` **on the host** (mode 0600), generated by `ps prod install`; template in `.env.prod.example` |
| what every key means, and how production differs from local | [config/production.md](config/production.md) |

In practice you rarely need the values themselves: `ps prod status`, `ps prod
logs`, `ps prod psql` and friends read them for you. Prefer running those over
reading the file, and never echo a value from it into a commit, a log or an
issue comment.
...
- [docs/pending-workflow-changes/issue-203-decision-id-ci.md](docs/pending-workflow-changes/issue-203-decision-id-ci.md) — broadens `task-4.2-occupancy.md`'s `jobs.test` pytest invocation to also collect `scripts/tests/` (no DB needed), which `task-4.2-occupancy.md` never covered — `test_doc_graph.py` and the new `test_decision_ids.py` (decision-record ID collision/frontmatter/cross-reference check) have been running locally only.
