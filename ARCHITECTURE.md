...
- **Current content**: still the old BI-dashboard-generation UI/flows and branding (page title, nav links) from the source project, deliberately untouched by Phase 1 (out of scope — see AGENTS.md). This means the dashboard as it stands today is not yet a usable inmo-tool UI — it's inherited scaffolding waiting on Phase 2 (candidate list #19, map #43, property detail page #44) to replace the UI content and Phase 4 (#24) to replace the LLM flow catalog. Ad-hoc execution is live again (issue #244, [D-038](docs/decisions/D-038-adhoc-etl-run-lock.md)): `etl/manual_trigger.py` polls `etl_manual_trigger`, so `POST /api/etl/run` inserts a trigger (all connectors, or one via `connector_name`) and the "Ejecutar ahora" button on `/admin/fuentes/[name]` (moved from `/etl/connectors` in issue #642 P1) and the global "Ejecutar todo ahora" on the `/admin/fuentes` list header (moved from `/etl` in issue #642 P2) drive it; `GET /api/etl/run?id=` reports status. The old source-project "force ETL resync" (watermark) semantics stay gone.

**Admin information architecture** — six sections, and no `/etl` tree ([D-168](docs/decisions/D-168-admin-six-sections-etl-tree-deleted.md), issue #642): Estado (`/admin`, the landing — per-source health, avisos, colas, crawl rollups) · Fuentes (`/admin/fuentes[/name]`) · Actividad (`/admin/actividad`, plus the run drill-down at `/admin/actividad/run/[id]`) · Revisión (`/admin/dedup` + `/admin/clasificacion` under one tab) · LLM (`/admin/llm`) · Configuración (`/admin/config`). `/admin/diagnostics` and `/admin/extension` are off-strip deep links owned by Fuentes. Every retired path (`/etl`, `/etl/[id]`, `/etl/salud`, `/etl/extension`, `/etl/connectors`, `/etl/captura`) is a wire-level 308 from `dashboard/next.config.js`'s `redirects()` — never a page-level redirect stub, which Next serves as a 200 with no `Location` and which would still count as a route.
