...
- **Current content**: still the old BI-dashboard-generation UI/flows and branding (page title, nav links) from the source project, deliberately untouched by Phase 1 (out of scope — see AGENTS.md). This means the dashboard as it stands today is not yet a usable inmo-tool UI — it's inherited scaffolding waiting on Phase 2 (candidate list #19, map #43, property detail page #44) to replace the UI content and Phase 4 (#24) to replace the LLM flow catalog. Its "force ETL resync" button used to insert into `etl_manual_trigger`, but nothing in the new connector orchestrator polls that table — `POST /api/etl/run` now returns `501` rather than a false success; use `ps connector run` instead.
