...
| [D-009](docs/decisions/D-009-restart-burst-guard.md) | A full connector sweep (scheduler startup, `--once` with no `--connector`) is skipped if a completed run finished within `etl.min_restart_sweep_interval_seconds` (default 900s) — guards against a crash-loop hammering every site on every restart. |
...
| [D-008](docs/decisions/D-008-skip-if-seen-opt-in.md) | Skip-if-seen (`Connector.min_refetch_interval_seconds`) defaults to 0 (always fetch) for every connector. Opt in per connector only with a documented reason; never skip a listing with no stored price or a disagreeing discovery-time price (`discovered_prices()`). |
