...
- **Removed entirely**: the "haul" concept (anonymous multi-tenant scrape collections), API-key auth, `haul-history.js` and its local-storage scrape history UI, the signup/limit-reached/no-key/haul-expired popup states. None of this applies to a tool with exactly one owner and one backend.
