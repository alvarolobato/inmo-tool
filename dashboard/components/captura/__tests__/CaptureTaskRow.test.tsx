...
describe("CaptureTaskRow", () => {
  it("renders the label, last-done note and an active button when not muted", () => {
    render(<CaptureTaskRow task={task()} muted={false} lastDone="nunca" lastRunAt={null} onExecute={vi.fn()} />);
  ...
  it("calls onExecute with the task on click", async () => {
    ...
    render(<CaptureTaskRow task={task()} muted={false} lastDone="nunca" lastRunAt={null} onExecute={onExecute} />);
  ...
  it("renders no checkbox of its own (issue #559 — selection moved to ConnectorSection's always-visible checklist)", () => {
    render(<CaptureTaskRow task={task()} muted={false} lastDone="nunca" lastRunAt={null} onExecute={vi.fn()} />);
    expect(screen.queryByTestId("captura-task-check-t1")).toBeNull();
