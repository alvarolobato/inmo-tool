...
describe("worklistMatchKey", () => {
  ...
  it("preserves path case (asset ids can be case-sensitive)", () => {
    expect(
      worklistMatchKey("https://alisedainmobiliaria.com/inmueble/AbC123"),
    ).toBe("alisedainmobiliaria.com/inmueble/AbC123");
  });
});
...
describe("portalForUrl", () => {
  it("maps a known capture host to its portal", () => {
    expect(
      portalForUrl("https://www.alisedainmobiliaria.com/inmueble/ANT1"),
    ).toBe("aliseda");
    expect(portalForUrl("https://www.idealista.com/inmueble/1/")).toBe(
      "idealista",
    );
  });
  ...
  it("matches subdomains of a capture host", () => {
    expect(
      portalForUrl("https://foo.alisedainmobiliaria.com/inmueble/ANT1"),
    ).toBe("aliseda");
  });
...
describe("firstPendingUrl — human-paced 'Siguiente' advance (issue #254)", () => {
  ...
  it("returns null when the only non-pending rows are stale", () => {
    expect(
      firstPendingUrl([row(1, "https://a/inmueble/1", "stale")]),
    ).toBeNull();
  });

  it("returns null when nothing is pending", () => {
    expect(
      firstPendingUrl([row(1, "https://a/inmueble/1", "captured")]),
    ).toBeNull();
...
describe("pendingUrls — the batch queue the extension sweeps (issue #262)", () => {
  ...
  it("returns every pending URL in list order, skipping other statuses", () => {
    ...
    expect(pendingUrls(rows)).toEqual([
      "https://a/inmueble/2",
      "https://a/inmueble/4",
    ]);
  });
...
describe("selectNextPendingUrls — auto-driver next batch (issue #424)", () => {
  ...
  it("sorts an unparseable/absent createdAt to the back", () => {
    ...
    expect(selectNextPendingUrls(withBad, { idealista: 0 }, 99)).toEqual([
      "good",
      "bad",
    ]);
  });
  ...
  it("force (dueOnly=false) includes not-due and unknown portals", () => {
    const dueRanks = { idealista: 0, aliseda: 1, altamira: 2 };
    expect(selectNextPendingUrls(items, dueRanks, 99, false)).toHaveLength(
      items.length,
    );
  });
...
describe("isCapturePortal — extension-only gate (issue #454)", () => {
  ...
  it("rejects cimenta2 — fetched by the ETL, not the extension", () => {
    // The whole point of #454: cimenta2 must never appear on the guided-capture
