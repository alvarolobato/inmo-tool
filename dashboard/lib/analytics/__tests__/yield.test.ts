...
import { computeYield, DEFAULT_FINANCING, DEFAULT_OPERATING_COST_PCT } from "../yield";
...
describe("computeYield", () => {
  ...
  it("matches manual reference calculation", () => {
    ...
    expect(assumptions!.financing_is_default).toBe(false);
  ...
  it("falls back to default assumptions when profile has none set", () => {
    ...
    const assumptions = result.assumptions_used;
    expect(assumptions).not.toBeNull();
    expect(assumptions!.financing_is_default).toBe(true);
  ...
  it("uses actual carrying costs (IBI + community fee) instead of the assumed operating-cost percentage when the source publishes them", () => {
    ...
    expect(withActuals.assumptions_used!.annual_carrying_costs_eur).toBeCloseTo(1460, 6);
    // Lower actual carrying costs than the assumed 25% -> a HIGHER net yield.
    expect(withActuals.net_yield_pct!).toBeGreaterThan(withoutActuals.net_yield_pct!);
  });

  it("treats a partially-known carrying cost (only IBI, no community fee) as actual, not a blend with the assumed percentage", () => {
    const result = computeYield(
      { purchase_price: 200000, province: "Madrid" },
      assumptionRent(800),
      { financing: { down_payment_pct: 20, rate_pct: 0, term_years: 25 } },
    ...
    expect(result.assumptions_used!.annual_carrying_costs_eur).toBeCloseTo(600, 6);
  });

  it("respects an itp_pct_override passed through thesis_params.acquisition_costs", () => {
    const result = computeYield(
      { purchase_price: 200000, province: "Madrid" },
      assumptionRent(800),
