...
/**
 * The portal *names* the browser extension can capture — the set form of
 * {@link CAPTURE_PORTALS}. `capture_worklist` is the extension's queue, so this
 * is the authoritative allow-list of portals that may appear on the guided
 * capture surface or be seeded into that table (issue #454).
 */
export const CAPTURE_PORTAL_NAMES: readonly string[] = CAPTURE_PORTALS.map(
  (p) => p.portal,
);

/**
 * Is `portal` an extension-capturable portal — i.e. does it belong in the
 * guided-capture worklist at all? (issue #454)
 *
 * A connector fetched over HTTP by the ETL (e.g. cimenta2, pulled via aura) is
 * NOT extension-capturable: the extension never drains its worklist, so any
 * `capture_worklist` rows for it are vestigial and misleading (0/N forever).
 * Its health lives in Data Health, never on the guided-capture list.
 */
export function isCapturePortal(portal: string): boolean {
  return CAPTURE_PORTAL_NAMES.includes(portal);
}
