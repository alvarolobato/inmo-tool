...
| `enabled = false` | Connector is skipped entirely for the run — never resolves a scope, never calls `discover()`, gets no `connector_run_results` row at all (not a failure; an operator turned it off). This is the lever the connector-management UI (#100) exposes. |
| `geography_override` set, `enabled` true/default | Used **instead of** the profile union for this connector only — e.g. an operator broadening ingestion ahead of profiles that don't exist yet. Same `{center, radius_km}` shape as a profile's geography. Malformed values fall back to the profile-derived default rather than erroring the run. |
| `filters` set | A JSONB bag of connector-specific native filters (e.g. `{"min_rooms": 2}`), merged onto whichever base scope (override or profile-derived) applies. Additive by design — see the per-site filter findings below for why this isn't a fixed column per site. |
