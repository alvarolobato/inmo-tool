...
**Schema superset vs. `property_web_scraper` (issue #76)**: `city`, `province`, `postal_code` (all `TEXT`), `m2_plot` (`NUMERIC(12,2)`, plot/land area distinct from `m2_built` — sized for rural fincas, not just urban flats), and `features` (`TEXT[]`, e.g. `{terraza,trastero,aire_acondicionado}`) were added as a structured superset of the reference project's field model — see [`connectors.md`](connectors.md#reusing-property_web_scraper--the-reference-project-for-this-domain) for the adaptation approach. **Not yet populated by any live connector as of this issue** — Fotocasa and Milanuncios both already parse city/province/postal_code internally and currently discard it into the flattened `address` string instead; wiring them to populate these columns is the Fotocasa/Milanuncios retrofit issues' job (#77/#78), not this one. `features` has no producer at all yet on any connector — the column exists so that retrofit work has somewhere to land it, not because anything writes to it today.
