...
**Schema superset vs. `property_web_scraper` (issue #76)**: `city`, `province`, `postal_code` (all `TEXT`), `m2_plot` (`NUMERIC(12,2)`, plot/land area distinct from `m2_built` — sized for rural fincas, not just urban flats), and `features` (`TEXT[]`, e.g. `{terraza,trastero,aire_acondicionado}`) were added as a structured superset of the reference project's field model — see [`connectors.md`](connectors.md#reusing-property_web_scraper--the-reference-project-for-this-domain) for the adaptation approach. **Population status as of issue #77**: Fotocasa now populates `city`/`province`/`postal_code`/`m2_plot` (it already parsed these internally and previously discarded them into the flattened `address` string instead — issue #77 promoted them to real columns). Milanuncios doesn't yet (issue #78, not done). `features` has no producer on either connector yet — Fotocasa's detail JSON doesn't carry the same boolean-amenity shape property_web_scraper's mapping assumes, and mapping it correctly needs its own investigation; the column exists so that work has somewhere to land, not because anything writes to it today.
...
**`operation`** (`TEXT NOT NULL DEFAULT 'sale' CHECK (operation IN ('sale', 'rent'))`, issue #76): both live connectors only ever discover sale listings today (Fotocasa's discovery URL is `/comprar/...`, Milanuncios's is `venta-de-pisos-en-...`), so this column has been invisible until now — added ahead of need, defaulted to `'sale'`, specifically so the trivial backfill (every existing/future row from the current two connectors is unambiguously a sale) doesn't turn into an emergency migration once a rental-comp connector (issue #31) needs to distinguish rentals from sales in the same tables. Issue #31's implementer should confirm this representation still fits before ingesting rental data — it hasn't been exercised against real rental listings yet.
