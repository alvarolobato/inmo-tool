...
**Extract (#28) is the fourth and last property-level flow** — `assessment_type = 'extract'`, same `loadPropertyListings()` shape as the other three. It fills the gap where a private-seller listing never structured `m2_built`/`m2_useful`/`rooms`/`bathrooms`/`floor`/`has_elevator` as a site field but says them in free text. Its output is written ONLY to `ai_assessment.result`, never merged into `property`'s own columns — provenance stays separate on purpose, so "what a connector structured" and "what an LLM inferred from prose" never get confused if the two ever disagree. `needsExtraction()`'s cost-control gate checks every field this flow can fill EXCEPT `m2_useful`: every connector hardcodes that column to `NULL` and this flow never writes it back to `property`, so including it in the gate would keep it permanently open (fixed in review of PR #180). **Not yet consumed anywhere**: neither `lib/filtering/scope-query.ts`'s hard filters nor `lib/candidates.ts`'s `loadFlags` card-badge query reads `assessment_type = 'extract'` rows — tracked as a fast-follow, issue #182 (flagged per #28's own "Additional Context", since #18/the hard-filter engine had already merged by the time #28 landed).
