...
**`operation`** (`TEXT NOT NULL DEFAULT 'sale' CHECK (operation IN ('sale', 'rent'))`, issue #76): both live connectors only ever discover sale listings today (Fotocasa's discovery URL is `/comprar/...`, Milanuncios's is `venta-de-pisos-en-...`), so this column has been invisible until now — added ahead of need, defaulted to `'sale'`, specifically so the trivial backfill (every existing/future row from the current two connectors is unambiguously a sale) doesn't turn into an emergency migration once a rental-comp connector (issue #31) needs to distinguish rentals from sales in the same tables. Fotocasa hardcodes `'sale'` (URL-pattern reasoning only, honestly caveated as such in-code). Milanuncios derives it per-listing from `category.slug` (`venta-*` → sale, `alquiler-*` → rent, issue #78) rather than a blanket hardcode — deliberately more defensive, since Milanuncios is a general classifieds site where a stray miscategorized listing is a real possibility in a way it likely isn't for a dedicated real-estate portal. Issue #31's implementer should confirm this representation still fits before ingesting rental data — it hasn't been exercised against real rental listings yet.
