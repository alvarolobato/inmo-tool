...
> ## REVISED (2026-08-20) — see [D-141](D-141-pisos-detail-fetch-for-reference-agency.md)
>
> "No detail fetch" is no longer true: `fetch_detail()` now makes one real
> request per listing, but ONLY to recover `reference_code`/`contact_raw`
> (issue #628) — neither is published anywhere in the search card or its
> JSON-LD, confirmed by a fresh live spike. Every other field this
> decision documents (price, rooms, baths, m², floor, coordinates,
> photos) is unchanged, still read from the search page with zero extra
