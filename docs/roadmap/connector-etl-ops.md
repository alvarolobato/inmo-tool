...
- The connectors page (`dashboard/app/etl/connectors/page.tsx`) renders a `ScopeSummary` per connector — plain-language "this is what will actually run next" (derived-from-profiles / explicit-override / capture-only / none), closing the exact gap issue #96 was filed about.
...
3. **No connector-level "last successful run" surfaced on the management page itself** — it's on `/etl` (RunList), but an owner deciding "should I disable Solvia" is looking at the connectors page, not correlating with a separate run list by name.
...
- `dashboard/app/etl/page.tsx` — `RunList` + `EvolutionCharts`, **live-polling** (not just a static history table), success-rate and fetch-rate KPIs.
...
4. Rewrite `POST /api/etl/run` to insert into `etl_manual_trigger` (optionally scoped to one `connector_name`) instead of returning 501; add a small "Ejecutar ahora" control per connector on `dashboard/app/etl/connectors/page.tsx` (§1) and one global "Ejecutar todo ahora" on `/etl`.
...
- Review queue: `suggested_merge` (status `pending`/`confirmed`/`rejected`/`conflict`, `match_basis`, `confidence`, `detail`, `resolved_at`), `dashboard/app/admin/dedup/page.tsx` + `SuggestionQueue.tsx`, confirm/reject/resolve-conflict API routes and CLI equivalents (`ps dedup confirm|reject|resolve-conflict`).
...
3. **The review queue has no priority ordering or batching aid.** `SuggestionQueue.tsx` (21-line page, thin wrapper) presumably lists pending suggestions in a fixed order — worth confirming whether high-confidence-but-not-quite-auto-merge suggestions (e.g. `photo_hash` ratio 0.95–0.999) are surfaced ahead of low-confidence `fuzzy` suggestions, since the owner's review time is the scarce resource, not engine compute.
...
2. **Sort/prioritize the review queue by confidence** (or at minimum group by `match_basis`) so the owner works through the highest-value suggestions first — a UI-only change to `SuggestionQueue.tsx`'s query, contingent on confirming today's order is in fact unordered/creation-order.
