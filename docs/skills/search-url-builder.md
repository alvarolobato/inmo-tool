...
| **aliseda** | `<comunidad>/<provincia>` path slugs from `provinces.ts`. Radius→province is **always broader** → geography **always loosened**. Point outside every box → drop geo segments + flag. | One task per **property type** (type is a path segment). `piso→pisos` owner-confirmed, other plurals guessed → flag. | `precio=<min>-<max>` (hyphen range, min defaults to 0), plus `subtipo=<code>` (`piso=36` confirmed; others omitted + flagged). Size has no confirmed grammar → dropped + flagged. |
...
- **Aliseda** — grammar **owner-confirmed 2026-08-05** (issue #277 fix). The
  builder emits
  `/comprar-viviendas/<tipo-plural>/<comunidad>/<provincia>?subtipo=<code>&precio=<min>-<max>`.
  Confirmed example: an Estepona-area piso ≤ 200 000 € →
  `https://www.alisedainmobiliaria.com/comprar-viviendas/pisos/andalucia/malaga?subtipo=36&precio=0-200000`.
  Confirmed tokens: base path `comprar-viviendas`, `piso→pisos`, `subtipo=36`
  (piso), `precio` hyphen range with min defaulting to `0`. **Guessed** (and
  flagged as loosened when used): plural segments for non-piso types
  (`aticos`/`chalets`/…), `subtipo` codes for non-piso types (omitted, not
  guessed). Province slugs come from `provinces.ts` (bounding boxes for málaga
  and sevilla, both `andalucia`).
