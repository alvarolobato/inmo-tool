...
| **[../roadmap/llm-batching-plan.md](../roadmap/llm-batching-plan.md)** | Round-2 structural plan: merging condition+location+opportunity into one `triage` call, N-property batching (design, limits, salvage rules, advisory-lock constraints), and why the profile-eligibility gate the owner asked for already exists (verified, 80% of ingest already excluded). Phased, each phase a PR. | Working on assessment call structure, prompt-version bumps, or asked to cut LLM calls further. |
