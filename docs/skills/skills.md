...
| **[captura-execution.md](captura-execution.md)** | Top-level guided-capture EXECUTION page (`dashboard/app/captura/`): pick a profile → per-portal pre-filtered search + worklist progress → "Abrir búsqueda". Composes `/api/profiles/[id]/search-urls` (#267) + `/api/etl/worklist` (#260) via the pure `lib/captura-view.ts`. Setup stays in `/etl` (D-044). Issue #268. | Building on the Captura page, the per-portal card, or the search-url ⋈ worklist join; deciding execution-vs-setup placement. |
